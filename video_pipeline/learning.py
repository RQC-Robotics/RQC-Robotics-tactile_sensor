...
print('input chain shape: ', pressure_shape, '\noutput chain shape: ',
      pressure_shape)
...
def iter_train(train_dataset, test_dataset, model, epochs, optimizer, criterion, pbar):
    for epoch in range(epochs):
        train_loss = fit_epoch(model,
                                ...
                                tr['batch_size'],
                                device)
        test_loss = eval_epoch(model, test_dataset, criterion, chain_len, tr['batch_size'], device)
...
# %%
with tqdm(total=tr['n_epochs'], desc="Learning", unit='epoch', ncols=140) as pbar:
    for i, h in iter_train(train_dataset,
                        test_dataset,
                        model=model,
                        epochs=tr['n_epochs'],
                        optimizer=optim,
                        criterion=loss_fn, pbar=pbar):
        history.append(h)
...
res = {'train': {'loss': train_loss[np.argmin(test_loss)]}, 'test': {'loss': min(test_loss)}}
