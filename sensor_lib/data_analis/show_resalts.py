...
def get_dic(n_ga_mas,n_fi_mas,path_true,path_pred,l=100000,interval=None):
  dic={}
  if interval == None:
    l2 = np.load(path_pred+str(n_ga_mas[0])+'g_'+str(n_fi_mas[0])+'fi').size[0]
    interval = (l-l2,l)

  for n_ga in n_ga_mas:
    mas = np.load(path_true+str(n_ga)+'_'+str(l)+'.npy', mmap_mode='r')
    m=mas[interval[0]:interval[1]]
    m=m.astype(np.float32)
    n_images, X, Y = m.shape
    mt = tf.constant(m,dtype=tf.float32)
    mt = tf.slice(mt, [0, int(X/6.0), int(Y/6.0)], [n_images, int(X*(1.0 - 2.0/6.0)), int(Y*(1.0 - 2.0/6.0))])
    m = mt.numpy()
