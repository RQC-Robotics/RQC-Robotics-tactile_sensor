...
def generate_pressure_map(n_images,
                          n_gauses,
                          x=97,
                          y=97,
                          part='fresh_gauss.npy',
                          size_g=6):
    hat_mat = round_fun((x, y), (int(x / 2), int(x / 2)),
                        lambda r: hat(r, int(x / 3)))
...
def sim_on_gpu(part,
               ...
               n_angles=4,
               fwhm=10,
               batch_size_preproc=128,
               size=None,
               test_size=None,
               max_possible_size=70000,
               n_del=1):
    with open(part, 'rb'
             ) as f:    # /content/drive/MyDrive/Colab_projects/fresh_gauss.npy
        mas = np.load(f)
    if size == None:
