...
def gaussian_func_flat(g_param, vec_mat):
    ...
    f = tf.vectorized_map(lambda r: tf.transpose(r) @ M @ r, x, fallback_to_while_loop=False)
    f = tf.reshape(f,[-1])
...
def generate_multi_gaussian_flat(gaus_data,vec_mat):
    ...
    gauses = tf.vectorized_map(lambda g_param : gaussian_func_flat(g_param , vec_mat),
                               params)
...
def generate_pictures(gaus_data,vec_mat):
  ...
  pictures = tf.vectorized_map( lambda data: generate_multi_gaussian_flat(data,vec_mat),
                               gaus_data, fallback_to_while_loop=False)
...
def fiber_real_sim(pressure_mat, cfg):
    ...
    if m==None:
      ...
      pressure_mat_angl=tf.keras.layers.RandomRotation(
                          ...
                          seed=None, fill_value=0.0)(pressure_mat3)
    ...
    pressure_tensor2 = tf.slice(pressure_tensor, [0, int(X/6.0), int(Y/6.0)], [n_images*m, int(X*(1.0 - 2.0/6.0)), int(Y*(1.0 - 2.0/6.0))])
